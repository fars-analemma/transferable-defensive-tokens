#!/bin/bash
# Evaluate direct-copy DefensiveTokens (Llama-3 -> Llama-3.1) on AlpacaFarm.
set -e
...
MODEL=$BASE/DefensiveToken/meta-llama/Llama-3.1-8B-Instruct-5DirectCopyTokens

set -a
source $BASE/.env 2>/dev/null || true
set +a

echo "=== [Llama-3.1 + Direct Copy DT from 3] Attack + Benign ==="
python $BASE/aligndeftoken/evaluation/inference_engine.py \
    ...
    --output_path $RESULTS/llama31_directcopy_from_3_all.json

echo "=== Direct copy eval (3->3.1) done ==="
