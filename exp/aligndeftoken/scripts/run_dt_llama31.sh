#!/bin/bash
# Job 4: Run DefensiveTokens on Llama-3.1-8B-Instruct (attack + benign).
set -e
...
MODEL=$BASE/DefensiveToken/meta-llama/Llama-3.1-8B-Instruct-5DefensiveTokens

echo "=== [Llama-3.1 + DT] Attack + Benign ==="
python $BASE/aligndeftoken/evaluation/inference_engine.py \
    ...
    --output_path $RESULTS/llama31_dt_all.json

echo "=== Llama-3.1 DefensiveTokens done ==="
